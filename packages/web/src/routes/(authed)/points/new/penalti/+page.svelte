<script lang="ts">
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      toast.success(m.points_penalti())
      goto('/points')
    } catch (err) {
...
<div class="mx-auto max-w-lg space-y-4">
  <div class="flex items-center gap-2">
    ...
    <h1 class="text-2xl font-bold">{m.penalti_form_title()}</h1>
  </div>

  <Card.Root>
    <Card.Header>
      <Card.Title>{m.points_penalti()}</Card.Title>
      <Card.Description>{m.point_type_penalti_desc()}</Card.Description>
    </Card.Header>
    <Card.Content>
      <form onsubmit={handleSubmit} class="space-y-4">
        ...
        <div class="space-y-1.5">
          <Label for="reason">{m.penalti_form_incident()}</Label>
          <Textarea
            id="reason"
