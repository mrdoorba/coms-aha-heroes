...
import type { FileAppealInput, ResolveAppealInput, ListAppealsInput } from '~/shared/schemas/appeals'
...
export async function resolveAppeal(
  ...
  const result = await appealsRepo.getByIdWithDetails(appealId, ctx.tx)
...
export async function listAppeals(
  ...
  const { rows, total } = await appealsRepo.listByAchievement(
    achievementId,
