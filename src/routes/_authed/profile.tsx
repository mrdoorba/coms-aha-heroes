...
function ProfileSkeleton() {
  return (
    <div className="mx-auto max-w-lg space-y-5 p-4 pt-6 animate-pulse">
      <div className="h-48 rounded-2xl bg-primary/10" />
  ...
  return (
    <div className="mx-auto max-w-lg space-y-4 p-4 pb-24 pt-5 md:pb-8">
        ...
        <div className="px-5 pb-5">
          ...
          <div className="relative -mt-10 mb-3 flex justify-between items-end">
            <div className="relative">
              <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl bg-card text-2xl font-bold text-primary ring-4 ring-card shadow-lg">
                {user.avatarUrl ? (
                  <img
                    src={user.avatarUrl}
                    alt={user.name}
              ...
              <button
                type="button"
                className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-md hover:bg-primary/80 transition-colors"
          ...
          {/* Name + email */}
          <h1 className="text-lg font-extrabold text-foreground leading-tight">{user.name}</h1>
      ...
      {(activity as ActivityItem[]).length > 0 && (
        <div className="rounded-2xl bg-card border border-border overflow-hidden shadow-card">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <h2 className="text-sm font-bold text-foreground">{m.profile_recent_points()}</h2>
            <Link
              to="/points"
          ...
          <div className="divide-y divide-border/50">
            {(activity as ActivityItem[]).map((item) => (
              <Link
                ...
                to="/points/$id"
                params={{ id: item.id }}
                className="flex items-center justify-between px-4 py-3 text-sm hover:bg-primary/4 transition-colors min-h-[44px]"
              >
      ...
      <div className="space-y-2.5">
        <Link
          to="/change-password"
          className="flex w-full items-center gap-3 rounded-2xl bg-card border border-border px-4 py-3.5 text-sm font-semibold text-foreground hover:bg-primary/4 transition-colors shadow-card min-h-[52px]"
        >
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-card border border-border px-4 py-3.5 shadow-card min-h-[52px]">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/10">
        ...
        <Button
          variant="outline"
          className="w-full justify-start gap-3 rounded-2xl border-red-200/70 bg-card py-3.5 text-red-600 hover:bg-red-50 hover:text-red-700 hover:border-red-300 dark:border-red-900/50 dark:hover:bg-red-900/20 dark:hover:border-red-800/60 min-h-[52px] shadow-card"
          disabled={loggingOut}
...
function StatCard({
  ...
  return (
    <div className="flex flex-col items-center gap-2 rounded-2xl bg-card border border-border p-3.5 shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(0,0,0,0.12)]">
