...
> **Heroes-side adoption SHIPPED 2026-05-03** (PR #2 on `mrdoorba/coms-aha-heroes`, deploy run `25267737871`). Steps 1-5 of this document executed end-to-end: app.css unified with tokens, chrome migrated to `@coms-portal/ui/chrome`, AccountWidget mounted in both ServiceBar and MobileTopBar, four lifted Heroes chrome files deleted, `/logged-out` route added for the post-portal-logout landing, `PUBLIC_APP_ORIGIN` wired in `deploy.yml`. Drizzle migration `0010_dear_kabuki` applied to staging + prod adding `session.portal_role` + `session.apps` populated by the broker exchange. Step 6 (flag-gating) skipped per Heroes pre-real-users posture. Heroes is now the first pillar of the unified COMS superapp running on shared chrome + widget. The body of this document below remains as the canonical reference for future H-app onboardings.
>
> **Pre-shipped status (2026-05-03):** Heroes-side adoption was unblocked by Spec 06 PR F shipping 2026-05-03 (commit `8f13c64`); portal-side rev3 feature work is complete.
>
> **Spec 06 PRs A → F shipped portal-side end-to-end (2026-04-30 → 2026-05-03).** PR E commit `212e103`, CI run `25265594527`; PR F commit `8f13c64`. The schema change is in: `identity_users.email` and `identity_users.personal_email` were dropped and replaced by the multi-row `identity_user_emails` table; OTP-based personal-email auth, login + profile + admin UIs, sign-out-everywhere, active-sessions panel, and the super-admin one-time login link are all live in production. Webhook payloads (`user.provisioned`, `user.updated`) carry the additive `emails: UserEmailEntry[]` field via `@coms-portal/shared` v1.5.0. Existing Heroes consumer compiles unchanged — the field is purely additive, no Heroes-side code change required. The widget still receives a scalar `email` from `/api/userinfo` (derived as workspace > personal-primary > first-personal per Spec 06 §Q8a); Heroes consumes only that scalar and is unaffected. **`super_admin` stays portal-internal** — `coms-shared` `PORTAL_ROLES` remains `['employee', 'admin']`; no Heroes-side enum change needed.
>
> **PR F (the spec-update sweep) shipped 2026-05-03.** Heroes-side rev3 adoption begins per this document and §Appendix A of `spec-03-user-identity-alias-layer.md`.
>
> Owner directive: PR F has finalised spec text → Heroes-side rev3 adoption begins now. Wipe-and-reprovision is the locked path (vs. alias backfill) — pre-real-users on Heroes makes the wipe a no-op. See `spec-00-implementation-timeline.md` for the at-a-glance status of the entire Rev 3 program. The body of this document below remains the canonical reference for Heroes' Rev 3 adoption.
>
> ---
>
...
- **Spec 03 portal-side has shipped (2026-04-28); Spec 03b test-gate cleared (2026-04-29).** Heroes-side adoption per §Appendix A of `spec-03-user-identity-alias-layer.md` is unblocked; both teams can sequence the cutover whenever capacity opens. Staging will pick up the post-Spec-03 release on the next push to `main` once CI confirms green. The new endpoints (resolve-batch, GET config, app-token middleware) are live on `main`.
- **Spec 03 §F display-name fallback**: portal's `/api/userinfo` reads `identity_users.name` directly. With Spec 03 shipped, the `user_aliases` table is now populated (one primary alias per active user via the backfill in migration `0024`). Updating `/api/userinfo` to read from `user_aliases WHERE is_primary = true` (with `ORDER BY created_at DESC LIMIT 1` belt-and-suspenders for the two-step rename window) is a small follow-up commit; the widget consumes the userinfo response as-is and needs no widget-side change.
- **`coms-shared` v1.4.0** is the latest, adding the four Spec 03 event names (`alias.resolved`, `alias.updated`, `alias.deleted`, `app_config.updated`) and an optional `appConfig` field on `UserProvisionedPayload`. Additive; existing consumers continue to compile. Heroes' webhook consumer for Spec 03 needs this version. Bump Heroes' `@coms-portal/shared` pin to `#v1.4.0`.
