...
## Status — 2026-05-03 (Specs 01 + 02 + 03 + 03b + 03c + 06 portal-side shipped end-to-end; Spec 01 + Spec 02 Phase 2/3/4 Heroes-side adoption SHIPPED 2026-05-03 — Heroes runs on shared chrome + tokens + widget. Next focus: **Spec 03 Heroes-side cutover (Deploy A/B/C)**)
...
| 01 | Shared Account Widget | Portal + Heroes | ✅ Portal-side shipped 2026-04-28; ✅ Heroes-side shipped 2026-05-03 (PR #2, deploy `25267737871` — chrome migration + AccountWidget mount + migration `0010` for session.portal_role/apps + /logged-out route) | ✅ Done | Yes — UX surface |
| 02 | Design System (skeleton + spec) | Portal + Heroes | ✅ Phases 1+2+3+4 shipped end-to-end across portal (2026-04-28/29) + Heroes (Phase 4 2026-04-30 commit `b7b7431`; Phases 2/3 2026-05-03 PR #2 — app.css unified with `@coms-portal/design-tokens` v1.1.0); Phase 5 deferred | ✅ All current phases done | No — deferred until trigger |
...
- **2026-05-03 — Spec 01 + Spec 02 Heroes-side adoption SHIPPED:** PR #2 on `mrdoorba/coms-aha-heroes` merged; deploy run `25267737871` cleared all four jobs (build & push, staging migrate + 0% deploy, manual approval, prod migrate + 100% deploy) in ~5 minutes. Heroes' chrome migrated to `@coms-portal/ui/chrome` (the four lifted Heroes-local files — ServiceBar/Sidebar/TopBar/MobileNav — deleted); AccountWidget mounted in ServiceBar + MobileTopBar right slots; app.css unified with `@coms-portal/design-tokens` v1.1.0 (semantic `:root` and `.dark` blocks stripped — tokens owns them, Heroes-only entries retained); `@coms-portal/{shared@v1.5.0, design-tokens@v1.1.0, ui@v1.2.0, account-widget@v0.2.0}` pinned. Drizzle migration `0010_dear_kabuki` adds `session.portal_role` + `session.apps` columns populated by the broker exchange — surfaces widget data on `locals.user` without per-request portal RTTs. New `/logged-out` route for post-portal-logout landing (clears heroes cookie + renders splash). `PUBLIC_APP_ORIGIN` wired in `deploy.yml` for both staging + prod. Heroes is now the first pillar of the unified COMS superapp. Live prod smoke tests green: `/api/healthz` ok, `/logged-out` clears cookie + renders, `/dashboard` 302 with deep-link encoded against `PUBLIC_APP_ORIGIN`.
