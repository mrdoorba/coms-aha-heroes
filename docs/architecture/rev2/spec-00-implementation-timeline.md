...
>
> **Last updated:** 2026-04-27 (H3 closed end-to-end; portal team confirmed `via:oidc` traffic for `app:heroes`; Heroes Day-30 cleanup landed same day)
...
| All specs deployed | Final audit: confirm all `PORTAL_*_SECRET` env vars unset on both sides. | **Heroes side fully complete 2026-04-27** — Day-30 cleanup mission merged same day H3 closed (soak window waived by mission lead). Audit grep zero matches across `*.ts`/`*.tf`/`*.json`/`*.yml`/`.env*`. `PORTAL_INTROSPECT_SECRET`, `PORTAL_WEBHOOK_SIGNING_SECRET`, `PORTAL_BROKER_SIGNING_SECRET` and `PORTAL_TOKEN_AUDIENCE` removed from `.github/workflows/deploy.yml`. Production deploy on commit `b01023a` succeeded; Secret Manager secrets `portal-introspect-secret` / `portal-webhook-signing-secret` / `portal-broker-signing-secret` deleted from project `fbi-dev-484410`. Portal-side audit pending portal team's own Day-30 mission. |
